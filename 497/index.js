...
function solution(records) {
    ...
    records.forEach((item, index) => {
        if (typeof item !== "object" || item === null) {
            ...
            for (const key in item) {
                if (!allIndexes.has(key)) {
                    allIndexes.set(key, new Map());
                }
                const valueSet = allIndexes.get(key);

                if (!valueSet.has(item[key])) {
                    valueSet.set(item[key], new Set());
                }
                valueSet.get(item[key]).add(index);
            }
        }
    });
    ...
    //                     }
    //                     const arr = intersectGroups.get(intSecHash).patterns.concat(p1.patterns).concat(p2.patterns);
